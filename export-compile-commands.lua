...
local function parseDefinition(def)
	local p = string.find(def, ".*_PCH=\"?.*PCH.hpp\"")
	if p ~= nil then
		local essentialPart = string.match(def, "\"(.+)\"")
		if essentialPart ~= nil then
			local defStart = string.find(def, "=\"?.+\"")
			if defStart ~= nil then
				if startsWith(essentialPart, "include/") then
					essentialPart = string.sub(essentialPart, string.len("include/") + 1)
				end
